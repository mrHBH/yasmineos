export default class DemoCLI {
	...
	enterKey() {
		...
		// // remove the old span and create a new one with the cursor
		//  const span = document.createElement("span");
		// 		span.setAttribute("data-cli-cursor", this.cursor);		
		// 		span.innerHTML = this.cursor;
	...
	removelastChar() {
    this.removeCursor();
    //keep removing last char until it's not a space
    //loop through all the spans and remove the last one until it's not a space
    const lastChar = this.container.children[this.container.children.length - 1];
	...
	removeCursor() {
     if (this.container.children.length > 0) {
      const lastChar = this.container.children[this.container.children.length - 1];
      if (lastChar.getAttribute("data-cli-cursor")) {
        lastChar.remove();
      }}
