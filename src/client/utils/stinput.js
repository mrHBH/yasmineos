					...
					class StInput {
						...
						constructor(element) {
							...
							if (!element) {
								element = window;
								console.warn(
									"'element' not provided to StInput; Will use 'window' instead, which is not always recommended."
								);
							}
						...
						get mouseDelta() {
							...
							// return mouse delta
							return new Point(
								this._mousePos.x - this._mousePrevPos.x,
								this._mousePos.y - this._mousePrevPos.y
							);
						}
						...
						get mouseMoving() {
							return (
								this._mousePrevPos && !this._mousePrevPos.equals(this._mousePos)
							);
						}
						...
						mousePressed(button = 0) {
							...
							return Boolean(
								this._mouseState[button] && !this._mousePrevState[button]
							);
						}
						...
						keyReleased(key) {
							if (key === undefined) throw new Error("Invalid key code!");
							return Boolean(
								!this._keyboardState[key] && this._keyboardPrevState[key]
							);
						}
						...
						get anyKeyDownKey() {
							for (var key in this._keyboardState) {
								if (this._keyboardState[key]) {
									//  return key;
									//return the key name, not the code
									return key;
						...
						down(code) {
							return Boolean(
								this._getValueWithCode(code, this.mouseDown, this.keyDown)
							);
						}
						...
						pressed(code) {
							return Boolean(
								this._getValueWithCode(code, this.mousePressed, this.keyPressed)
							);
						}
						...
						_onMouseDown(event) {
							event = this._getEvent(event);
							if (
								this.enableMouseDeltaWhileMouseWheelDown &&
								event.button === this.MouseButtons.middle
							) {
								event.preventDefault();
							}
