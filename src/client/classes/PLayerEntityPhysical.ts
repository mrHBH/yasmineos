...
class PhysicalCharacterController extends Component {
	...
	//	this.world.removeBody(this.leftFootBody);
//		this.world.removeBody(this.rightFootBody);
		this.world.removeBody(this.leftHandBody);
		this.world.removeBody(this.rightHandBody);
	...
	AddPhysicsBody() {
		...
		const colliderShapeTop = new CANNON.Cylinder(0.10, 0.10, 0.9, 8);
		...
		this.body.addShape(colliderShape, new CANNON.Vec3(0,0.07, 0.2));
...
//		this.world.addBody(this.leftFootBody);
	...
	UpdateFSM(input: any) {
		...
		if (
			...
			(this.AnimationFSMService_.state.value == "Walking" ||
				this.AnimationFSMService_.state.value == "Running" ||
				this.AnimationFSMService_.state.value == "SlowWalking" ||				this.AnimationFSMService_.state.value == "Pushing")
		) {
			this.AnimationFSMService_.send("STOP");
		}
	...
	AddColliders() {
		...
		const geometry = new THREE.CylinderGeometry(39, 39, 160, 32);
		...
		spineCollider.position.add(new THREE.Vector3(0, 0, 0));
	...
	_FindIntersections(
		...
		const nearby = grid.FindNearbyEntities(2).map((e) => e.entity)
			 .filter((e: any) => _isCollidable(e)).map ((e: any) => e.Mesh);
	...
	Update(timeInSeconds: number) {
		...
		//apply a force to the body to move it in the direction of forward
		// this.body.velocity.x=forward.x * acc.z/2;
		// this.body.velocity.z=forward.z * acc.z/2;

		// // this.body.applyLocalForce(
		// // 	new CANNON.Vec3(forwardDir.x, forwardDir.y, forwardDir.z),
		// // 	new CANNON.Vec3(forward.x / 1000, forward.y / 1000, forward.z /1000)
		// // );

		//this.body.velocity.set(forward.x, forward.y, forward.z);
		...
		// if (this.input._keys.forward) {
		// 	this.body.velocity.x = (forwardDir.x * acc.z) / this.body.mass;
		// 	this.body.velocity.z = (forwardDir.z * acc.z) / this.body.mass;
		...
		//get distance bewtween foot and ground
		
		//chekc if the up velocity is less than 0.1 when not jumping
		
		//if so, then set the velocity to 0.1
		...
		//check if foot is on ground , else move up a bit
		...
		//project a ray from the foot to the ground
	...
	//	controlObject.position.set(pos.x, this.body.position.y, pos.z);
		controlObject.position.set(this.body.position.x, this.body.position.y, this.body.position.z);
