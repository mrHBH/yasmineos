...
class WorldEntity {
	...
	UpdateTransform(transformData: transformData) {
	//	console.log("--------------------------------------> transformData: ");
	//	console.log(typeof transformData);
		this.state_ = transformData[0];
		this.position_ = vec3.fromValues(
			...(transformData[1] as [number, number, number])
		);
		this.rotation_ = quat.fromValues(
			...(transformData[2] as [number, number, number, number])
		);
...
type transformData = [string, [number, number, number], [number, number, number, number]];

export { WorldEntity, transformData };
